...
class Baseline(nn.Module):
    ...
    def __init__(self, cfg, num_classes, last_stride, model_path, neck, neck_feat, model_name, pretrain_choice):
        ...
        self.match = cfg.INPUT.SIM_OCCLUD
        ...
        if model_name == 'ResNet50':
            self.in_planes = 256
            self.big_planes = 1024
            self.base = HighResolutionNet(cfg)

        if pretrain_choice == 'imagenet' and model_name == 'HRNet32':
    ...
    def forward(self, x):
        y_part, y_global, y_fore, clustering_feat_map, part_pd_score = self.base(x)
        # torch.Size([256, 768, 1, 1]) torch.Size([256, 256, 1, 1]) torch.Size([256, 256, 1, 1])
        ...
        if self.training:
            ...
            return cls_score_part, cls_score_global, cls_score_fore, y_part, y_global, y_fore, part_pd_score
        else:
            if self.neck_feat == 'after':
                ...
                if self.arm:
                    ...
                    return feat_fore, feat_part.view(feat_part.shape[0], self.part_num - 1, self.in_planes), \
                               visible_part, clustering_feat_map
                else:
                    print('Test without Arm')
                    return torch.cat((feat_global, feat_fore, feat_part), 1), clustering_feat_map
