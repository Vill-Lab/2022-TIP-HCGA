...
def make_loss(cfg, num_classes):
    parsing_criterion = CrossEntropy(ignore_label=cfg.TRAIN.IGNORE_LABEL)  # parsing loss
    ...
    def loss_func(cls_score_part, cls_score_global, cls_score_fore, y_part, y_global, y_fore, part_pd_score, cls_target,
                  part_target):
        loss = cfg.SOLVER.CrossE_G_WEIGHT * xent(cls_score_global, cls_target) + \
               cfg.SOLVER.CrossE_F_WEIGHT * xent(cls_score_fore, cls_target) + \
               cfg.SOLVER.CrossE_P_WEIGHT * xent(cls_score_part, cls_target) + \
               cfg.SOLVER.PARSING_LOSS_WEIGHT * parsing_criterion(part_pd_score, part_target) + \
               cfg.SOLVER.Tri_G_WEIGHT * triplet(y_global, cls_target)[0] + \
               cfg.SOLVER.Tri_F_WEIGHT * triplet(y_fore, cls_target)[0]
        return loss, xent(cls_score_global, cls_target), xent(cls_score_fore, cls_target), \
               xent(cls_score_part, cls_target), parsing_criterion(part_pd_score, part_target)
